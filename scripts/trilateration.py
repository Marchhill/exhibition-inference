...
def mse(x, beacon_locations, beacon_distances):
    calculated_distances = np.array([np.linalg.norm(x - beacon_locations[i]) for i in range(len(beacon_locations))])
...
def main():
    # Testing the code
    beacon_locations = np.array([[0,0,0], [10, 0, 0], [10, 10, 0], [0, 10, 0]])
    # Actual location of point is at [7,4,3]
    beacon_distances = np.array([8.6, 5.83, 7.35, 9.69])
    print(trilaterate(beacon_locations, beacon_distances)) # Result generated is [ 6.99620322  4.00005823 -2.99673958]
